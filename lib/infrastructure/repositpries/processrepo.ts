...
export class ProcessRepo extends BaseRepository implements IProcessRepo {
  ...
  async createMaterial(ownerId: string, name: string, description: string, imageName: string, bleDeviceId: string, userInfo: any): Promise<CreateMaterialSdo> {
  ...
  async uploadMaterialImage(id: string, imageName: string, image: any): Promise<boolean> {
    const files: any[] = [image];
    const fileNames: string[] = [imageName];
    
    const res: ApiResult = await this.api.uploadFiles(API.UPLOAD_IMAGES(id), files, fileNames);
